...
class startThreadRatos extends Thread
{
    private $config;
    ...
    function run()
    {
        ...
        $a = new aStar($this->config['inicio'], $this->config['rato'], $this->config['x'], $this->config['y'], $this->config['num_obstaculos']);
        $a->exibirLabirinto();
    }
}
...
for ($i = 0; $i < 5; $i++) {
    $thread = (new startThreadRatos($config))->start();
}
